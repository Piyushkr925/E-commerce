...
const UserManagement = () => {
  ...
  const getUser = async () => {
    await axios
      .get("http://192.168.1.172:5000/user/getAllUserDetail/")
      .then((result) => {
  ...
  // SEARCH API on Username
  const search = async (e) => {
    const key = e.target.value;
    if (key) {
      let srdata = await fetch(`http://192.168.1.172:5000/user/search/${key}`);
  ...
  const deleteUser = async (id) => {
    await axios
      .delete("http://192.168.1.172:5000/user/deleteUser/" + id)
      .then((result) => {
    ...
    const sortingAsc = (event) => {
      ...
      axios
        .get("http://192.168.1.172:5000/user/sortAsc")
        .then((res) => {
    ...
    const sortingDesc = (event) => {
      setSelectedOption(event.target.value);
      axios
        .get("http://192.168.1.172:5000/user/sortdesc")
        .then((res) => {
          setData(res.data.Data);
        })
        .catch((err) => console.log(err));
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
            <div className="flex container px-6 py-8 justify-between">
              <h3 className="text-4xl font-bold text-gray-700">User</h3>
              {/* ****************************PAGINATION*************************** */}
                    ...
                    <table className="min-w-full">
                      <thead>
                        <tr>
                          ...
                          <th className="px-4 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">
                            Email
                          </th>
                          <th className="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">
                            Role
